...
BACKUP="/storage/.config/extlinux.conf.backup-restore-verbose-$(date +%Y%m%d-%H%M%S)"
cp "$EXTLINUX" "$BACKUP" || {
  remount_flash_readonly
...
awk '
  ...
  /^[[:space:]]*APPEND[[:space:]]/ {
    ...
    printf " loglevel=7 systemd.show_status=1 rd.systemd.show_status=1\n"
    next
  }
...
' "$EXTLINUX" > /tmp/extlinux.conf.restore || {
  ...
  remount_flash_readonly
  log "ERROR: could not generate verbose extlinux configuration."
  exit 1
}
...
cp /tmp/extlinux.conf.restore "$EXTLINUX" || {
  rm -f /tmp/extlinux.conf.restore
  remount_flash_readonly
  log "ERROR: could not install verbose extlinux configuration."
  exit 1
}
rm -f /tmp/extlinux.conf.restore "$OVERLAY"
remount_flash_readonly
log "Early splash overlay removed and verbose tty0 boot messages restored."
...
sync
log "DONE: original boot restored with verbose reboot messages. Reboot to test."
exit 0
